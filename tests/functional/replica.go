...
func (c *testConfig) startReplicas() {
...
func (c *testConfig) stopReplicas() {
	for replica := range c.Replicas {
		rep := replica
		go func(replica string) {
			c.StopTestReplica(replica)
		}(rep)
	}
}

func (c *testConfig) cleanReplicaDirs() {
	for replica := range c.Replicas {
		rep := replica
		go func(replica string) {
...
func (c *testConfig) CreateAutoGeneratedSnapshots(count int, replica string, replicaCount int) {
	for i := 0; i < count; i++ {
		c.StopTestReplica("172.18.0.112")
		c.verifyRWReplicaCount(1)
		time.Sleep(2 * time.Second)
		go c.startTestReplica("172.18.0.112", "172.18.0.112"+"vol", true)
		c.verifyRWReplicaCount(2)
	}
}
